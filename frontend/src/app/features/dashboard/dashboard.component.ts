import { Component, inject, OnInit, signal } from '@angular/core';
import { DecimalPipe } from '@angular/common';
import { RouterLink } from '@angular/router';
import { ApiService } from '../../core/services/api.service';
import { Employee, Payslip } from '../../core/models/kpay.models';

@Component({
    selector: 'app-dashboard',
    standalone: true,
    imports: [DecimalPipe, RouterLink],
    template: `

    <div class="space-y-6">
      <div class="page-head">
        <div>
          <h1 class="page-title">Tableau de bord</h1>
          <p class="page-subtitle">Aperçu de la paie et des effectifs de votre organisation.</p>
        </div>
      </div>

      @if (errorMessage()) {
        <div class="alert alert-error">{{ errorMessage() }}</div>
      }

      <!-- Métriques KPIs -->
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="kpi-card">
          <div class="flex items-center justify-between">
            <span class="kpi-label">Employés actifs</span>
            <svg class="w-5 h-5 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
          </div>
          <p class="kpi-value">{{ activeEmployees().length }}</p>
          <p class="mt-1 text-xs text-slate-400">{{ employees().length - activeEmployees().length }} inactif(s)</p>
        </div>

        <div class="kpi-card">
          <div class="flex items-center justify-between">
            <span class="kpi-label">Bulletins ce mois</span>
            <svg class="w-5 h-5 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
          </div>
          <p class="kpi-value">{{ payslips().length }}</p>
          <p class="mt-1 text-xs text-slate-400">période en cours</p>
        </div>

        <div class="kpi-card">
          <div class="flex items-center justify-between">
            <span class="kpi-label">Masse salariale brute</span>
            <svg class="w-5 h-5 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
          </div>
          <p class="kpi-value">{{ totalGrossPay() | number }} F</p>
          <p class="mt-1 text-xs text-slate-400">cumul brut</p>
        </div>

        <div class="kpi-card">
          <div class="flex items-center justify-between">
            <span class="kpi-label">Net versé</span>
            <svg class="w-5 h-5 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
          </div>
          <p class="kpi-value">{{ totalNetPay() | number }} F</p>
          <p class="mt-1 text-xs text-slate-400">cumul net</p>
        </div>
      </div>

      <!-- Actions Rapides -->
      <div class="card p-5 flex flex-wrap items-center justify-between gap-4">
        <h2 class="card-title">Actions rapides</h2>
        <div class="flex flex-wrap gap-2">
          <a routerLink="/employees" class="btn btn-secondary">
            Gérer les employés
          </a>
          <a routerLink="/payroll" class="btn btn-primary">
            Lancer un calcul de paie
          </a>
        </div>
      </div>
    </div>
  `
})
export class DashboardComponent implements OnInit {
    private apiService = inject(ApiService);

    employees = signal<Employee[]>([]);
    payslips = signal<Payslip[]>([]);
    errorMessage = signal('');

    ngOnInit(): void {
        this.loadData();
    }

    loadData(): void {
        this.apiService.getEmployees().subscribe({
            next: (data) => this.employees.set(data),
            error: (err) => {
                this.employees.set([]);
                this.errorMessage.set('Impossible de charger les employés.');
            }
        });

        this.apiService.getPayslips().subscribe({
            next: (data) => this.payslips.set(data),
            error: () => {
                this.payslips.set([]);
                this.errorMessage.set('Impossible de charger les bulletins.');
            }
        });
    }

    activeEmployees(): Employee[] {
        return this.employees().filter((e) => e.is_active);
    }

    totalGrossPay(): number {
        return this.payslips().reduce((acc, p) => acc + p.gross_amount, 0);
    }

    totalNetPay(): number {
        return this.payslips().reduce((acc, p) => acc + p.net_amount, 0);
    }
}
