        ...
        <table class="w-full text-left text-sm text-slate-600">
          <thead class="bg-slate-50 text-slate-700 uppercase font-semibold text-xs border-b border-slate-200">
            <tr>
              ...
              <th class="px-6 py-3.5">Manager (N+1)</th>
          ...
          <tbody class="divide-y border-slate-100">
            @for (emp of employees(); track emp.id) {
              <tr class="hover:bg-slate-50 transition">
                ...
                <td class="px-6 py-4 text-slate-500">{{ managerName(emp.manager_id) }}</td>
          ...
          <div class="bg-white rounded-2xl p-6 w-full max-w-lg shadow-2xl space-y-4">
            ...
            <form (ngSubmit)="saveEmployee()" class="space-y-4">
              ...
              <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label class="block text-xs font-medium text-slate-700 mb-1">Manager (N+1)</label>
                  <select [(ngModel)]="newEmployee.manager_id" name="manager" class="w-full px-3 py-2 rounded-lg border border-slate-300 text-sm focus:ring-blue-500 focus:border-blue-500">
